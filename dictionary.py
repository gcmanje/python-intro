...
#print(students[11])
sorted_by_eng =sorted(students, key= lambda x: x['english'], reverse=False)
sorted_by_total =sorted(students, key= lambda x: x['total'], reverse=False)

for s in sorted_by_eng:
    print(s["english"])

top_ten = sorted_by_total[0:1]
for x in top_ten:
     print(x['names'], x['total'])

top_ten= sorted_by_total[-1:]
for x in top_ten:
    print(x['names'], x['total'])
...
sorted_by_chem=sorted(students, key= lambda x: x['chemistry'], reverse=False)
top_male_in_chem= sorted_by_chem[1:]
for x in top_male_in_chem:
    if x['chemistry']=='M':
        print(x['names'])

bottom_female_in_eng=sorted_by_eng[-1:]
for x in bottom_female_in_eng :
    if x['gender'] == 'F':
        print(x['names'])